...
from db.data_collect import data_collect_service
import json
...
async def get_settings(user_id: str):
    ...
    try:
        settings = await data_collect_service.get_value(f"settings_{user_id}")
        if settings:
            return settings
...
async def update_settings(user_id: str, settings: Dict[str, Any]):
    ...
    try:
        await data_collect_service.update_value(
            f"settings_{user_id}",
            settings,
            "json"
        )
...
async def get_image(user_id: str):
    ...
    try:
        image = await data_collect_service.get_value(f"image_{user_id}")
        if image:
            return {"image": image}
        return {"image": None}
    except Exception as e:
...
async def get_zoom(user_id: str):
    ...
    try:
        zoom = await data_collect_service.get_value(f"zoom_{user_id}")
        if zoom:
            return {"zoom": zoom}
...
async def get_values():
    """Get all values from the data collect"""
    try:
        # Initialize the service if needed
        await data_collect_service.initialize()
        
        # Get all values (already serialized by DataCollect class)
        values = await data_collect_service.get_all_values()
        return values
    except Exception as e:
...
async def update_value(data: Dict[str, Any]):
    ...
    try:
        ...
        await data_collect_service.update_value(key, value, data_type)
